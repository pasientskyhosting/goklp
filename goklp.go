...
Config file is required, named: goklp.ini or passed using --config=/path/to/file
  ...
  goklp_ldap_user_attr        = uid                                       (optional - default: uid)
...
type opts struct {
	...
	goklp_ldap_ssh_key_attr    string
	goklp_ldap_filter          string
  ...
  searchFilter := fmt.Sprintf("(%s=%s)", o.goklp_ldap_user_attr, o.username)
	...
	for _, server_url := range o.goklp_ldap_uris {
		q := query{
			...
			filter:     searchFilter,
			Attributes: []string{o.goklp_ldap_ssh_key_attr},
...
func parseConfigFile(configFile string, o *opts) error {
	...
	goklp_ldap_ssh_key_attr, exists := config[""]["goklp_ldap_ssh_key_attr"]
	if !exists {
		o.goklp_ldap_ssh_key_attr = "sshPublicKey"
	} else {
  ...
  goklp_ldap_filter, exists := config[""]["goklp_ldap_filter"]
	if !exists {
		...
		o.goklp_ldap_filter = goklp_ldap_filter
	}
